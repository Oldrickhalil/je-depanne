...
export default function AdminDashboard() {
  ...
  return (
    <div className="min-h-screen bg-[#070707] text-white p-6 md:p-12 font-sans pb-32">
      <div className="max-w-6xl mx-auto space-y-10">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
           ...
           <div className="flex bg-[#111] p-1 rounded-2xl border border-white/5">
              <button 
                ...
                className={`px-6 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${view === 'loans' ? 'bg-primary text-white shadow-lg shadow-primary/20' : 'text-gray-500 hover:text-white'}`}
              >
                Prêts
              </button>
              <button 
                onClick={() => setView('users')}
                className={`px-6 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${view === 'users' ? 'bg-primary text-white shadow-lg shadow-primary/20' : 'text-gray-500 hover:text-white'}`}
              >
                Utilisateurs
              </button>
           </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
           {[
             ...
             <div key={i} className="bg-[#0c0c0c] border border-white/5 p-6 rounded-[2rem] space-y-3">
           ...
           <div className="flex items-center justify-between border-b border-white/5 pb-6 gap-4">
              ...
              <div className="relative w-full max-w-xs">
                 ...
                 <input 
                   ...
                   className="bg-[#121212] border border-white/5 rounded-full py-2.5 pl-10 pr-6 text-[10px] font-bold uppercase tracking-widest focus:outline-none focus:ring-1 focus:ring-primary/50 transition-all w-full"
                 />
              </div>
           </div>
             ...
             <div className="space-y-4">
               {view === 'loans' ? (
                 filteredLoans.map((loan) => (
                   <div key={loan.id} className="flex flex-col md:flex-row md:items-center justify-between p-6 rounded-[2.5rem] bg-[#0c0c0c] border border-white/5 hover:border-primary/10 transition-all gap-6">
                      <div className="flex items-center gap-4">
                         <div className="w-12 h-12 rounded-xl bg-white/5 flex items-center justify-center text-primary font-black">
                           {loan.amount}
                         </div>
                         <div>
                            <p className="text-sm font-black uppercase tracking-tight text-white">{loan.user.firstName}</p>
                            <p className="text-[9px] text-gray-500 font-bold uppercase tracking-[0.2em]">{loan.user.email}</p>
                         </div>
                      </div>
                      ...
                      <div className="flex items-center gap-4">
                         ...
                         <button 
                            ...
                            className="px-4 py-2 bg-white/5 text-gray-400 text-[8px] font-black uppercase tracking-widest rounded-lg hover:bg-white/10 hover:text-white transition-all"
                         >
                            Détails
                         </button>
                      </div>
                 ...
                 filteredUsers.map((user) => (
                   <div key={user.id} className="flex flex-col md:flex-row md:items-center justify-between p-6 rounded-[2.5rem] bg-[#0c0c0c] border border-white/5 hover:border-primary/10 transition-all gap-6">
                      <div className="flex items-center gap-4">
                         <div className="w-12 h-12 rounded-xl bg-white/5 flex items-center justify-center">
                            <Users size={18} className="text-gray-500" />
                         </div>
                         <div>
                            <p className="text-sm font-black uppercase tracking-tight text-white">{user.firstName} {user.lastName}</p>
                            <p className="text-[9px] text-gray-500 font-bold uppercase tracking-[0.2em]">{user.email}</p>
                         </div>
                      </div>
                      ...
                      <div className="flex flex-wrap items-center gap-4">
                         <div className="flex gap-2">
                            ...
                            <span className={`text-[8px] font-black uppercase tracking-widest px-3 py-1.5 rounded-full ${user.hasDeposited ? 'bg-green-500/10 text-green-500' : 'bg-gray-500/10 text-gray-500'}`}>Dépôt</span>
                            <span className={`text-[8px] font-black uppercase tracking-widest px-3 py-1.5 rounded-full ${user.isInstalled ? 'bg-green-500/10 text-green-500' : 'bg-gray-500/10 text-gray-500'}`}>PWA</span>
                         </div>
                         
                         <div className="flex items-center gap-4 pl-4 border-l border-white/5">
                            <div className="text-right">
                               <p className="text-[8px] font-black text-gray-600 uppercase">Balance</p>
                               <p className="text-xs font-black">{user.wallet?.balance || 0} €</p>
                            </div>
                            {!user.kycVerified ? (
                              <button 
                                onClick={() => setSelectedKycUser(user)}
                                className="px-4 py-2 bg-amber-500/20 text-amber-500 text-[8px] font-black uppercase tracking-widest rounded-lg hover:bg-amber-500 hover:text-white transition-all"
                              >
                                Examiner KYC
                              </button>
                            ) : (
      ...
      {selectedKycUser && (
        <div className="fixed inset-0 z-[100] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-[#111] border border-white/10 rounded-[2.5rem] w-full max-w-4xl max-h-[90vh] overflow-y-auto custom-scrollbar p-8 md:p-10 space-y-8 relative shadow-2xl">
            <button 
            ...
            <div className="space-y-2">
               ...
               <p className="text-xs text-gray-500 font-bold uppercase tracking-[0.2em]">Client: {selectedKycUser.firstName} {selectedKycUser.lastName} • {selectedKycUser.email}</p>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
               <div className="space-y-6">
                  <div className="bg-[#161616] p-6 rounded-3xl border border-white/5 space-y-4">
                     <p className="text-[9px] font-black uppercase tracking-widest text-gray-500">Informations Déclarées</p>
                     <div>
                        <p className="text-[10px] text-gray-400 uppercase tracking-widest">Document</p>
                        <p className="text-base font-bold">{selectedKycUser.idType === 'passport' ? 'Passeport' : 'Carte d\'Identité'}</p>
                     </div>
                     <div>
                        <p className="text-[10px] text-gray-400 uppercase tracking-widest">Date de naissance</p>
                     ...
                     <div>
                        <p className="text-[10px] text-gray-400 uppercase tracking-widest">Adresse Déclarée</p>
               ...
               <div className="col-span-2 space-y-6">
                  {selectedKycUser.kycAddressProof && (
                     <div className="space-y-3">
                        <div className="flex items-center gap-2">
                           ...
                           <p className="text-xs font-black uppercase tracking-widest text-white">Justificatif de Domicile</p>
                        </div>
                        <div className="aspect-[21/9] relative rounded-[2rem] overflow-hidden border border-white/10 bg-[#161616] group">
                  ...
                  {selectedKycUser.kycRecto && (
                     <div className="space-y-3">
                        <div className="flex items-center gap-2">
                           <ShieldCheck size={16} className="text-primary" />
                           <p className="text-xs font-black uppercase tracking-widest text-white">Pièce d'Identité ({selectedKycUser.idType === 'passport' ? 'Page Info' : 'Recto'})</p>
                        </div>
                        <div className="aspect-[16/10] relative rounded-[2rem] overflow-hidden border border-white/10 bg-[#161616] group">
                  ...
                  {selectedKycUser.kycVerso && (
                     <div className="space-y-3">
                        <div className="flex items-center gap-2">
                           <ShieldCheck size={16} className="text-primary" />
                           <p className="text-xs font-black uppercase tracking-widest text-white">Pièce d'Identité (Verso)</p>
                        </div>
                        <div className="aspect-[16/10] relative rounded-[2rem] overflow-hidden border border-white/10 bg-[#161616] group">
                  ...
                  {!selectedKycUser.kycRecto && !selectedKycUser.kycAddressProof && (
                     <div className="h-64 flex flex-col items-center justify-center border-2 border-dashed border-white/5 rounded-[2rem] text-gray-500 gap-4">
                        <AlertCircle size={32} />
      ...
      {selectedLoan && (
        <div className="fixed inset-0 z-[100] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-[#111] border border-white/10 rounded-[2.5rem] w-full max-w-2xl max-h-[90vh] overflow-y-auto custom-scrollbar p-8 md:p-10 space-y-8 relative shadow-2xl">
            <button 
            ...
            <div className="space-y-2 border-b border-white/5 pb-6">
               <h2 className="text-3xl font-black uppercase tracking-tighter text-white">Détails du Prêt</h2>
               <p className="text-xs text-gray-500 font-bold uppercase tracking-[0.2em]">Client: {selectedLoan.user.firstName} {selectedLoan.user.lastName} • {selectedLoan.user.email}</p>
            </div>

            <div className="space-y-6">
               <div className="grid grid-cols-2 gap-4">
                  <div className="bg-[#161616] p-4 rounded-2xl border border-white/5">
                     <p className="text-[9px] text-gray-500 uppercase tracking-widest font-bold">Montant Demandé</p>
                     <p className="text-2xl font-black text-primary">{selectedLoan.amount} €</p>
                  </div>
                  <div className="bg-[#161616] p-4 rounded-2xl border border-white/5">
                     <p className="text-[9px] text-gray-500 uppercase tracking-widest font-bold">Durée & Taux</p>
                     <p className="text-lg font-black text-white">{selectedLoan.termMonths} mois <span className="text-xs text-gray-500 font-medium">@ 3%</span></p>
                  </div>
               </div>
                  ...
                  <div className="bg-[#161616] p-6 rounded-3xl border border-white/5 space-y-4">
                     <div className="flex justify-between items-end">
                        <div>
                           ...
                           <p className="text-2xl font-black text-white">{(selectedLoan as any).amountRepaid?.toFixed(2) || "0.00"} € <span className="text-sm text-gray-500">/ {(selectedLoan.amount * 1.03).toFixed(2)} €</span></p>
                        </div>
               ...
               <div className="bg-[#161616] p-6 rounded-3xl border border-white/5 space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-y-4 gap-x-2">
                     <div>
                        <p className="text-[9px] text-gray-500 uppercase tracking-widest">État Civil</p>
                        <p className="text-sm font-bold text-white capitalize">{selectedLoan.maritalStatus === 'single' ? 'Célibataire' : selectedLoan.maritalStatus === 'married' ? 'Marié(e) / Pacsé(e)' : selectedLoan.maritalStatus === 'divorced' ? 'Divorcé(e)' : selectedLoan.maritalStatus === 'widowed' ? 'Veuf/Veuve' : 'Non renseigné'}</p>
                     </div>
                     <div>
                        <p className="text-[9px] text-gray-500 uppercase tracking-widest">Situation Pro.</p>
                        <p className="text-sm font-bold text-white capitalize">{selectedLoan.employmentStatus === 'employed' ? 'Salarié(e)' : selectedLoan.employmentStatus === 'self_employed' ? 'Indépendant(e)' : selectedLoan.employmentStatus === 'unemployed' ? 'Sans emploi' : selectedLoan.employmentStatus === 'student' ? 'Étudiant(e)' : selectedLoan.employmentStatus === 'retired' ? 'Retraité(e)' : 'Non renseigné'}</p>
                     </div>
                     <div>
                        <p className="text-[9px] text-gray-500 uppercase tracking-widest">Revenus Mensuels</p>
                        <p className="text-sm font-bold text-white">{selectedLoan.monthlyIncome ? `${selectedLoan.monthlyIncome} €` : 'Non renseigné'}</p>
                     </div>
                     <div>
                        <p className="text-[9px] text-gray-500 uppercase tracking-widest">Crédits en cours</p>
                        <p className="text-sm font-bold text-white">{selectedLoan.hasExistingCredits ? 'Oui' : 'Non'}</p>
                     </div>
                     <div className="col-span-2">
                        <p className="text-[9px] text-gray-500 uppercase tracking-widest">Motif du prêt</p>
                        <p className="text-sm font-bold text-white capitalize">{selectedLoan.loanPurpose === 'emergency' ? 'Urgence' : selectedLoan.loanPurpose === 'purchase' ? 'Achat / Consommation' : selectedLoan.loanPurpose === 'travel' ? 'Voyage' : selectedLoan.loanPurpose === 'auto' ? 'Réparation Auto' : selectedLoan.loanPurpose === 'other' ? 'Autre' : 'Non renseigné'}</p>
                     </div>
                  </div>
               </div>
