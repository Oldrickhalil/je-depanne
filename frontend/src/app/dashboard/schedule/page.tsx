    ...
    <div className="max-w-4xl mx-auto space-y-10 animate-in fade-in duration-700 pb-20 relative">
      ...
      {successModal?.show && (
         <div className="fixed inset-0 z-[150] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
            <div className="bg-[#111] border border-white/10 rounded-[2.5rem] w-full max-w-md p-10 space-y-6 relative shadow-2xl animate-in zoom-in-95 duration-500 text-center overflow-hidden">
               ...
               <div className="space-y-4">
                  <h2 className="text-3xl font-black uppercase tracking-tighter text-white">Paiement Réussi</h2>
                  <p className="text-xs text-gray-400 font-bold uppercase tracking-widest leading-relaxed px-2">
      ...
      <section className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div className="space-y-1">
          <Link href="/dashboard/stats" className="flex items-center gap-2 text-[9px] font-black uppercase tracking-[0.3em] text-gray-500 hover:text-white transition-colors mb-4">
      ...
      {loading ? (
         ...
         <div className="space-y-10">
            ...
            <div className="space-y-6">
               <h3 className="text-[10px] font-black uppercase tracking-[0.3em] text-gray-500 pl-2">À Rembourser</h3>
               ...
               {activeLoans.length > 0 ? (
                  <div className="grid grid-cols-1 gap-6">
                     {activeLoans.map(loan => {
                        ...
                        return (
                           <div key={loan.id} className="bg-[#0c0c0c] border border-primary/20 p-8 rounded-[2.5rem] relative overflow-hidden shadow-2xl">
                              ...
                              <div className="relative z-10 flex flex-col md:flex-row justify-between gap-8">
                                 <div className="space-y-6 flex-1">
                                    <div>
                                       ...
                                       <p className="text-4xl font-black tracking-tighter text-white mt-1">{remainingBalance.toFixed(2)} €</p>
                                    </div>
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                       <div className="bg-[#161616] p-4 rounded-2xl border border-white/5">
                                          <p className="text-[8px] text-gray-500 uppercase tracking-widest font-bold">Mensualité</p>
                                          <p className="text-sm font-bold text-white">{monthlyPayment.toFixed(2)} € <span className="text-[9px] text-gray-500">/mois</span></p>
                                       </div>
                                       <div className="bg-[#161616] p-4 rounded-2xl border border-white/5">
                                          <p className="text-[8px] text-gray-500 uppercase tracking-widest font-bold">Durée totale</p>
                                          <p className="text-sm font-bold text-white flex items-center gap-1.5"><Calendar size={12} className="text-primary" /> {loan.termMonths} mois</p>
                                       </div>
                                    </div>
                                 </div>
                                 ...
                                 <div className="flex flex-col justify-end gap-3 min-w-[220px]">
                                    <button 
                                       ...
                                       className="w-full py-4 bg-white/10 text-white font-black text-[10px] uppercase tracking-widest rounded-2xl hover:bg-white/20 transition-all flex items-center justify-center gap-3 disabled:opacity-50 border border-white/5"
                                    >
                  ...
                  <div className="bg-[#111] border border-white/5 rounded-[2rem] p-8 text-center space-y-2">
                     <p className="text-xs font-black uppercase tracking-widest text-white">Aucun prêt en cours</p>
                     <p className="text-[10px] font-bold text-gray-500 uppercase tracking-widest">Vous n'avez aucune échéance à régler.</p>
                  </div>
               )}
            </div>
            ...
            {paidLoans.length > 0 && (
               <div className="space-y-6">
                  <h3 className="text-[10px] font-black uppercase tracking-[0.3em] text-gray-500 pl-2">Historique des remboursements</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                     {paidLoans.map(loan => (
                        <div key={loan.id} className="bg-[#111] border border-white/5 p-6 rounded-[2rem] flex items-center gap-4">
                           <div className="w-12 h-12 rounded-2xl bg-green-500/10 text-green-500 flex items-center justify-center shrink-0">
                              <CheckCircle2 size={18} />
                           </div>
                           <div>
                              <p className="text-sm font-black text-white uppercase tracking-tight">{(loan.amount * (1 + loan.interestRate)).toFixed(2)} €</p>
                              <p className="text-[9px] font-bold text-gray-500 uppercase tracking-widest mt-0.5">Payé et clôturé</p>
                           </div>
                        </div>
                     ))}
                  </div>
      ...
      {confirmModal?.show && (
         <div className="fixed inset-0 z-[100] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
            <div className="bg-[#111] border border-white/10 rounded-[2.5rem] w-full max-w-md p-8 space-y-6 relative shadow-2xl animate-in zoom-in-95 duration-300 text-center">
               <button 
               ...
               <div className="space-y-2">
                  <h2 className="text-2xl font-black uppercase tracking-tighter text-white">Confirmer le paiement</h2>
                  <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest leading-relaxed px-4">
                    Êtes-vous sûr de vouloir payer <span className="text-white">{confirmModal.amount.toFixed(2)} €</span> depuis votre solde disponible ?
                  </p>
               </div>

               <div className="bg-[#161616] p-4 rounded-2xl border border-white/5 text-center">
                 <p className="text-[9px] font-black uppercase tracking-widest text-gray-500">Type d'opération</p>
               ...
               <div className="flex gap-4 pt-2">
                 <button 
                   onClick={() => setConfirmModal(null)}
                   className="flex-1 py-4 bg-white/5 text-gray-400 font-black rounded-2xl hover:bg-white/10 hover:text-white transition-all uppercase text-[10px] tracking-widest"
                 >
                   Annuler
                 </button>
                 <button 
